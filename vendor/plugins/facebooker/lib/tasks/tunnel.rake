namespace :facebooker do
  ...
  namespace :tunnel do 
    ...
    task :config => :environment do
     ...
     @notification = "Starting tunnel #{@public_host}:#{@public_port} to 127.0.0.1:#{@local_port}"
     @notification << " using SSH port #{@ssh_port}" unless @ssh_port == 22
     @ssh_command = "ssh -p #{@ssh_port} -nNT -g -R 0.0.0.0:#{@public_port}:127.0.0.1:#{@local_port} #{@public_host}" 
    end
  end
end